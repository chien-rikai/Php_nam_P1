<form class="form-horizontal form_data" action="{{url('admin/user/'.$users->uid)}}" method="POST" enctype="multipart/form-data">
    @method('PATCH')
    @csrf
    <div class="form-group row">
      <label for="inputName" class="col-sm-2 col-form-label">{{ trans('message.fullname') }} (*)</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="fullname" name="fullname" value="{{$users->fullname}}" placeholder="{{ trans('message.fullname') }}">
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-10">
        <div class="form-check">
            <input class="form-check-input" value="1" type="radio" name="gender" {{($users->gender == 1) ? 'checked' : ""}}>
            <label class="form-check-label">{{ trans('message.male') }}</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" value="2" type="radio" name="gender"  {{($users->gender == 2) ? 'checked' : ""}}>
            <label class="form-check-label">{{ trans('message.female') }}</label>
        </div>
        <div class="form-check">
            <input class="form-check-input" value="3" type="radio" name="gender"  {{($users->gender == 3) ? 'checked' : ""}}>
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-10">
        <input type="date" name="birthday" class="form-control" id="date" value="{{$users->birthday}}">
    ...
    <div class="form-group row">
        ...
        <img class="img_form" src="{{checkUserAvatar($users->image, $users->gender)}}" alt="">
        <div class="input-group">
          <div class="custom-file">
            <input type="file" class="custom-file-input" value="{{$users->image}}" onchange="readURL(this);" id="image" name="image">
